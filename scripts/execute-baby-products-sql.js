...
async function clearAndAddBabyProducts() {
  console.log('🔧 Clearing all existing products and adding baby products...\n')
  ...
  // Step 2: Delete all products by ID
  ...
  // Step 3: Insert baby products in batches
  console.log('👶 Adding baby category products...\n')
  
  const batchSize = 10
  for (let i = 0; i < babyProducts.length; i += batchSize) {
    const batch = babyProducts.slice(i, i + batchSize)
    ...
    if (insertError) {
      ...
      console.error('Batch:', batch.map(p => p.name))
      return
    }
    
    console.log(`✅ Inserted batch ${Math.floor(i / batchSize) + 1}: ${insertedProducts?.length || 0} products`)
  }
  
  console.log('\n✅ Successfully inserted all baby products\n')
